        ...
        private void CellGUI(Rect cellRect, TreeViewItem<AssetInfo> item, Columns column, ref RowGUIArgs args)
        {
            ...
            switch (column)
            {
                ...
                case Columns.Version:
                    ...
                    {
                        GUIContent version = new GUIContent(item.Data.AssetSource == Asset.Source.RegistryPackage ? item.Data.InstalledPackageVersion() : item.Data.GetVersion());
                ...
                case Columns.Name:
                    ...
                    if (item.Data.Id > 0)
                    {
                        ...
                        else if (item.Data.AssetSource == Asset.Source.CustomPackage)
                        {
                            Texture folderIcon = EditorGUIUtility.IconContent("d_ModelImporter Icon").image;
                            if (folderIcon != null) GUI.DrawTexture(toggleRect, folderIcon);
                        }
                    }
                    else
