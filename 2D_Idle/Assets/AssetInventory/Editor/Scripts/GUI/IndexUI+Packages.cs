    ...
    public partial class IndexUI
    {
        ...
        private void DrawPackageDetails(AssetInfo info, bool showMaintenance = false, bool showActions = true, bool startNewSection = true)
        {
            ...
            if (!string.IsNullOrWhiteSpace(info.GetDisplayPublisher()))
            {
                if (info.PublisherId > 0)
                {
                    ...
                    if (GUILayout.Button(UIStyles.Content(info.GetDisplayPublisher()), UIStyles.wrappedLinkLabel)) Application.OpenURL(info.GetPublisherLink());
                    GUILayout.EndHorizontal();
                }
                else
                {
            ...
            if (_tab == 1 && (ShowAdvanced() || info.AssetSource == Asset.Source.Directory || info.AssetSource == Asset.Source.Archive)) GUILabelWithText("Indexed Files", $"{info.FileCount:N0}");
            if (ShowAdvanced() && info.ChildInfos.Count > 0) GUILabelWithText("Sub-Packages", $"{info.ChildInfos.Count:N0}");
        ...
        private void CreateAssetTree()
        {
            ...
            switch (_selectedMaintenance)
            {
                ...
                case 12:
                    List<int> duplicates = filteredAssets.Where(a => a.ParentId == 0 && a.ForeignId > 0).GroupBy(a => a.ForeignId).Where(g => g.Count() > 1).Select(g => g.Key).ToList();
                    filteredAssets = filteredAssets.Where(a => duplicates.Contains(a.ForeignId));
                    break;

                case 13:
                    filteredAssets = filteredAssets.Where(a => a.ParentId == 0 && a.Backup);
                    break;
                ...
                case 15:
                    filteredAssets = filteredAssets.Where(a => a.ParentId == 0 && a.AssetSource != Asset.Source.AssetStorePackage && a.AssetSource != Asset.Source.RegistryPackage && !a.Downloaded);
                    break;
            ...
            for (int depth = 1; depth <= maxChildDepth; depth++)
            {
                ...
                {
                    // remove items at existing positions and set correct depth
                    ...
                    // find item with id pair.Key and insert items afterwards
